...
class Captcha extends Database {
    ...
    async fillPeaper(count = this.height) {
        for (let i = 0; i < count; i++) {
            ...
            this.ctx.strokeStyle = `rgb(${generateRandomNumber(
                0,
                256
            )},${generateRandomNumber(0, 256)},${generateRandomNumber(0, 1)})`;
            this.ctx.beginPath();
    ...
    async create(opts = { buffer, width, height }, userAgent) {
        if (!userAgent)
            throw new AttributeRequireError("userAgent is must be require!");
        ...
        for await (let chunk of quiz) {
            let fontSize = await generateRandomNumber(
                Math.floor(this.height / 1.4 - 7),
                Math.floor(this.height / 1.4)
            );
    ...
    async check(token, answer, userAgent) {
        if (!userAgent)
            throw new AttributeRequireError("userAgent is must be required!");

		if (answer == undefined)
			throw new AttributeRequireError("\"answer\" param is required!");

        const { id, userAgent: tokenUserAgent } = JWT.verify(token);

		console.log(userAgent, tokenUserAgent);
        ...
        if (!quiz)
            throw new QuizNotFoundError(
                "Captcha not found in database please try restarting."
            );
